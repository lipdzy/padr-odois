...
function loadProductDetails(index) {
    ...
    // Atualizar valor da quantidade se o produto já estiver no carrinho
    updateQuantityDisplay(index);
}

// Função para atualizar o display de quantidade baseado no carrinho
function updateQuantityDisplay(productIndex) {
    const quantityValue = document.getElementById('quantityValue');
    if (!quantityValue) return;
    
    // Verificar se o produto já está no carrinho
    const existingItem = cartItems.find(item => item.index === productIndex);
    
    if (existingItem) {
        // Se o produto já estiver no carrinho, mostrar a quantidade atual
        quantityValue.textContent = existingItem.quantity;
    } else {
        // Se não estiver no carrinho, mostrar 0
...
function setupDetailPageEvents(productIndex) {
    ...
    // Diminuir quantidade - remove 1 item do carrinho
    newDecreaseBtn.addEventListener('click', function() {
        ...
        if (existingItemIndex !== -1) {
            // Se estiver no carrinho, diminuir a quantidade
            if (cartItems[existingItemIndex].quantity > 1) {
                // Reduzir quantidade
                cartItems[existingItemIndex].quantity--;
                quantityValue.textContent = cartItems[existingItemIndex].quantity;
                ...
                // Mostrar notificação
                showNotification(`Um ${produtos[productIndex].nome} removido do carrinho.`);
            } else {
                ...
                // Mostrar notificação
                showNotification(`${produtos[productIndex].nome} removido do carrinho.`);
            }
        } else {
            // Se não estiver no carrinho, mostrar mensagem
            showNotification("Este item não está no seu carrinho.");
        }
    });
    
    // Aumentar quantidade - adiciona 1 item ao carrinho
    newIncreaseBtn.addEventListener('click', function() {
        ...
        if (existingItemIndex !== -1) {
            ...
            // Não está no carrinho, adicionar como novo item
            cartItems.push({
                index: productIndex,
                name: produtos[productIndex].nome,
                price: produtos[productIndex].price,
                image: produtos[productIndex].image,
                quantity: 1
            });
        ...
        // Mostrar notificação
        showNotification(`Um ${produtos[productIndex].nome} adicionado ao carrinho.`);
    });
    
    // Adicionar ao carrinho - agora se comporta como "Verificar Carrinho"
    newAddToCartBtn.addEventListener('click', function() {
        // Abrir o carrinho para visualização
        openCart();
        
        // Alterar texto do botão para indicar sua nova função
        newAddToCartBtn.innerHTML = '<i class="fas fa-shopping-cart"></i> Ver Carrinho';
    });
}

// Função para adicionar produto ao carrinho a partir da página de detalhes (mantida para compatibilidade)
